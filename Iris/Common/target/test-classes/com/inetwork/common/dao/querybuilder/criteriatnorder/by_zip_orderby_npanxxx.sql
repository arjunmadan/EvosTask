...
INNER JOIN (SELECT DISTINCT npa,nxx,state FROM npa_nxx_zipcode_lerg AS nnz WHERE nnz.zip_code=:zip_code) AS nnz 
...
WHERE (tn.tn_status='Available' AND tn.protected_account_id IS NULL) AND rtx.tier_id IN (SELECT tier_id from account_tier_xref WHERE account_id =:account_id )
 ORDER BY tn.npa ASC,tn.nxx ASC,tn.block ASC
 LIMIT 100